...
int main(int argc , char ** argv) {
    ...
    const std::tuple<double, double> priceResult = monteCarlo.price();
    const double price = std::get<0>(priceResult);
    const double priceStdDev = std::get<1>(priceResult);

    
    PnlMat* past0 = pnl_mat_create(1, D);
  pnl_mat_set_row(past0, monteCarlo.model.S0, 0);

  const std::tuple<PnlVect*, PnlVect*> deltaResult =
      monteCarlo.delta(past0, 0.0);

  pnl_mat_free(&past0);
    PnlVect* deltaVect = std::get<0>(deltaResult);
    PnlVect* deltaStdDevVect = std::get<1>(deltaResult);

  
    PricingResults results(price, priceStdDev, deltaVect, deltaStdDevVect);
