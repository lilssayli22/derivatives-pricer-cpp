...
parser.add_argument("--compare-only", help="Only compare the results", action="store_true")
...
if args.price:
    price_or_hedge = Action.PRICE
elif args.hedge:
    price_or_hedge = Action.HEDGE
    if args.marketdir:
...
def compare_prices0(expected, obtained, name='price'):
    ...
    if obtained_std_dev is not None:
...
def compare_deltas(expected, obtained, name='delta'):
    ...
    if any(x is None for x in obtained_delta) or any(x is None for x in obtained_delta_std_dev):
        return (None, None)
    delta_same_interval = []
    delta_std_dev_ratio = []
...
def run_allexec_tests(action, all_test_cases):
    for exe in exec_folder.iterdir():
        # if not exe.match('**/exec_*'):
        #     continue
        if action == Action.PRICE and not exe.match('**/price0*'):
            continue
        if action == Action.HEDGE and not exe.match('**/hedge*'):
            continue
        exe_fullpath = exec_folder / exe
        run_singleexec_test(exe_fullpath, action, all_test_cases)


def run_singleexec_test(exe_fullpath: Path, action: Union[Action.PRICE, Action.HEDGE], all_test_cases: list[Path]):
    """Run all tests for a single executable

    Args:
        exe_fullpath: Full path of the executable
        action: Action.PRICE or Action.HEDGE
        all_test_cases: list of all test cases to run

    Raises:
        json.JSONDecodeError: when the executable does not return json output
    """
...
#compare results
def run_compare_results(action, all_test_cases):
...
if __name__ == "__main__":
    all_tests = get_all_test_cases(test_folder)
    if not compare_only:
        if execpath:
            run_singleexec_test(execpath, price_or_hedge, all_tests)
        else:
            run_allexec_tests(price_or_hedge, all_tests)
    run_compare_results(price_or_hedge, all_tests)
